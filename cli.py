...
from common import install_requests_cache, prepare, find_paths_for_fiat, ConversionPath, prepare_conversion_paths
...
def display_path_rates(path_rates: List[ConversionPath], limit_top_n=10) -> None:
    for num, path_rate in enumerate(path_rates):
        if num >= limit_top_n:
            break
        path_visual = '-'.join([conversion.from_currency for conversion in path_rate.conversions] + [path_rate.conversions[-1].to_currency])
