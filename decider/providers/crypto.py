...
class BinanceEdge(Edge):
    ...
    def __init__(self, from_: Node, to: Node, ticker: dict, market: dict, is_direct: bool):
...
def add_quotes_to_graph(tickers, markets, graph: Graph):
    ...
    for ticker in tickers.values():
        ...
        edge_direct = BinanceEdge(from_=base, to=quote, ticker=ticker, market=market, is_direct=True)
        graph.add(edge_direct)

        edge_reverse = BinanceEdge(from_=quote, to=base, ticker=ticker, market=market, is_direct=False)
